...
function renderWallets() {
    ...
    if (!wallets.length) { list.innerHTML = `<div class="hint">Belum ada wallet. Tambahkan sekarang.</div>`; home.innerHTML = `<div class="hint">Belum ada dompet.</div>`; }
    else { list.innerHTML = wallets.map(walletItemHTML).join(''); home.innerHTML = wallets.map(walletItemHomeHTML).join(''); }
    fillStatsWalletSelect();
}
function recentTxItemHTML(t) {
    const sign = t.type === 'in' ? '+' : '-'; const cls = t.type === 'in' ? 'plus' : 'minus';
    const w = wallets.find(x => x.id === t.walletId); const walletName = w ? w.name : '—';
    ...
    <div class="tx-info">
      ...
      <div><div class="tx-title">${cat}</div><div class="tx-meta">${fmtDate(t.date)} • ${walletName}${t.note ? ' • ' + t.note : ''}</div></div>
    </div>
...
function setTxType(t) {
    txType = t; $('#tagIn').style.outline = t === 'in' ? '2px solid rgba(34,197,94,.6)' : 'none';
    $('#tagOut').style.outline = t === 'out' ? '2px solid rgba(239,68,68,.6)' : 'none'; fillTxCategory();
}
function fillTxWallet() {
    const sel = $('#inWallet'); if (!wallets.length) { sel.innerHTML = `<option value="">(Belum ada wallet)</option>`; return; }
    sel.innerHTML = wallets.map(w => `<option value="${w.id}">${w.name}</option>`).join('');
}
function fillTxCategory(selectedValue = null) {
    const sel = $('#inCategory'); const type = (txType === 'in') ? 'income' : 'expense'; const list = cats[type] || [];
    if (!list.length) { sel.innerHTML = `<option value="">(Belum ada kategori)</option>`; $('#catHint').style.display = 'block'; }
    else { sel.innerHTML = list.map(c => `<option value="${c}">${c}</option>`).join(''); $('#catHint').style.display = 'none'; if (selectedValue && list.includes(selectedValue)) sel.value = selectedValue; }
}
...
function reverseTxEffect(t) { const w = wallets.find(w => w.id === t.walletId); if (!w) return; w.balance = Number(w.balance || 0) + (t.type === 'in' ? -t.amount : +t.amount); }
function applyTxEffect(t) { const w = wallets.find(w => w.id === t.walletId); if (!w) return; w.balance = Number(w.balance || 0) + (t.type === 'in' ? +t.amount : -t.amount); }
function applyEditWalletAdjustments(oldT, newT) { reverseTxEffect(oldT); applyTxEffect(newT); }

function saveTxFromModal() {
    const amount = Number($('#inAmount').value || 0), walletId = $('#inWallet').value, category = $('#inCategory').value, note = $('#inNoteTx').value.trim(), date = $('#inDate').value || new Date().toISOString().slice(0, 10);
    if (!amount || amount <= 0) { alert('Nominal harus > 0'); return; } if (!walletId) { alert('Pilih dompet'); return; }
    if (txMode === 'create') { const t = { id: uuid(), type: txType, amount, category, note, walletId, date }; txs.push(t); applyTxEffect(t); }
    else {
        const idx = txs.findIndex(x => x.id === editingTxId); if (idx < 0) { closeModal('#modalTx'); return; }
        const oldT = { ...txs[idx] }; const newT = { ...oldT, type: txType, amount, category, note, walletId, date }; applyEditWalletAdjustments(oldT, newT); txs[idx] = newT;
    }
...
function doDeleteTx(txId, silentClose = false) {
    const idx = txs.findIndex(t => t.id === txId); if (idx < 0) return; const t = txs[idx];
    if (!confirm('Hapus transaksi ini?')) return;
    reverseTxEffect(t); txs.splice(idx, 1); save(LS_TX, txs); save(LS_WALLETS, wallets); renderAll();
...
function fillStatsWalletSelect() {
    ...
    sel.innerHTML = [`<option value="all">Semua Dompet (${wallets.length})</option>`].concat(wallets.map(w => `<option value="${w.id}">${w.name}</option>`)).join('');
...
/* === Guard Export: tombol aktif + fallback alert jika CDN gagal === */
function bindExportButtonGuard() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-hourglass-half"></i> Memuat…';
    ...
    // Setelah 4.5 detik, aktifkan walau belum siap (klik akan beri alert)
    setTimeout(() => {
        if (!(window.__XLSX_READY__ && window.XLSX)) {
            btn.disabled = false;
            btn.innerHTML = initialText;
    ...
    btn.addEventListener('click', () => {
        if (!(window.__XLSX_READY__ && window.XLSX)) {
            alert("Modul Excel belum berhasil dimuat dari CDN. Coba lagi sebentar, atau pastikan koneksi/CDN tidak diblokir.");
            return;
        }
        exportToExcel();
    });
...
function exportToExcel() {
    if (!window.XLSX) {
        alert("Modul Excel belum siap. Coba lagi sebentar.");
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Jika sudah keburu ada XLSX (mis. loader di HTML), set flag
    if (window.XLSX) window.__XLSX_READY__ = true;
    ...
    ['#modalAddWallet', '#modalTx', '#modalCats'].forEach(sel => {
        const el = $(sel); if (!el) return; el.addEventListener('click', e => { if (e.target === e.currentTarget) closeModal(sel); });
    });
    ...
    $('#recentTx')?.addEventListener('click', e => {
        const row = e.target.closest('.tx'); if (!row) return; const txId = row.getAttribute('data-txid');
        const actEdit = e.target.closest('[data-action="edit"]'); const actDel = e.target.closest('[data-action="del"]');
    ...
    // Export Excel (guard + fallback alert bila CDN gagal)
